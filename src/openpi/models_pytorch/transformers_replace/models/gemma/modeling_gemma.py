...
class GemmaMLP(nn.Module):
    ...
    def forward(self, x, **kwargs):
        # Extract LoRA routing kwargs and pass through to all projections
        lora_kw = {}
        if "audio_mask" in kwargs:
            lora_kw["audio_mask"] = kwargs["audio_mask"]
        if "gradient_mode" in kwargs:
            lora_kw["gradient_mode"] = kwargs["gradient_mode"]

        down_proj = self.down_proj(
            self.act_fn(self.gate_proj(x, **lora_kw)) * self.up_proj(x, **lora_kw),
            **lora_kw,
        )
        return down_proj
...
class GemmaDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Extract LoRA kwargs to pass to both attention and MLP
        lora_kw = {}
        if "audio_mask" in kwargs:
            lora_kw["audio_mask"] = kwargs["audio_mask"]
        if "gradient_mode" in kwargs:
