"""Audio ablation for Stage 3 checkpoints.

Usage: CUDA_VISIBLE_DEVICES=X python scripts/diag_audio_ablation_stage3.py <checkpoint_path>
"""
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scripts/diag_audio_ablation_stage3.py <checkpoint_path>")
        sys.exit(1)
    ...
    # Match Stage 3 architecture: gemma_2b_lora for Gemma LLM
    ...
    mean_zero = np.mean(losses_zero)
    mean_rand = np.mean(losses_rand)
    delta = mean_zero - mean_real

    log(f"\n{'='*60}")
    log(f"AUDIO ABLATION RESULTS — {CKPT}")
    log(f"{'='*60}")
    log(f"Mean loss (real audio):   {mean_real:.4f}")
    log(f"Mean loss (zero audio):   {mean_zero:.4f}")
    log(f"Mean loss (random audio): {mean_rand:.4f}")
    log(f"Delta (zero - real):      {delta:+.4f}")
    log(f"")
    if delta > 0.5:
        log(f"CONCLUSION: Audio IS being used (delta = {delta:+.4f})")
    elif delta > 0.1:
        log(f"CONCLUSION: Audio has SOME effect (delta = {delta:+.4f})")
    else:
        log(f"CONCLUSION: Audio is IGNORED (delta = {delta:+.4f})")
